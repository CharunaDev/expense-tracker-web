...
import { MonthlyBudget, Category } from '../../models/expense.model';
...
export class BudgetsComponent implements OnInit {
  ...
  showModal = false;
  editingBudget: MonthlyBudget | null = null;
  ...
  loadBudgets() {
    this.apiService.getBudgets(this.selectedYear, this.selectedMonth).subscribe(budgets => {
  ...
  calculateTotals() {
    ...
    this.totalSpent = this.budgets.reduce((sum, b) => sum + (b.spentAmount || 0), 0);
    this.remainingBudget = this.totalBudget - this.totalSpent;
  }

  getRemainingBudget(budget: MonthlyBudget): number {
    return budget.budgetAmount - (budget.spentAmount || 0);
  }

  getPercentage(budget: MonthlyBudget): number {
    const percentage = ((budget.spentAmount || 0) / budget.budgetAmount) * 100;
    return Math.min(percentage, 100);
  }

  openBudgetModal(budget?: MonthlyBudget) {
  ...
  editBudget(budget: MonthlyBudget) {
